...
var PzpSendMessage = function () {
    ...
    this.sendConnectedApp = function (address, message) {
        if (address && message) {
            if (PzpObject.connectedWebApp.hasOwnProperty (address)) {
                message.from = PzpObject.convertSessionIdToFriendlyName(message.from);
                message.resp_to = PzpObject.convertSessionIdToFriendlyName(message.resp_to);
                message.to = PzpObject.convertSessionIdToFriendlyName(message.to);

                if(message.payload && message.payload.method && message.payload.method.indexOf("servicefound") > -1) {
                    message.payload.params.serviceAddress = PzpObject.convertSessionIdToFriendlyName(message.payload.params.serviceAddress);
                }
