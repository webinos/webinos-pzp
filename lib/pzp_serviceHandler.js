...
var PzpServiceHandler = function () {
    ...
    PzpObject.setServiceConfiguration = function(serviceName, params){
        ...
        if(foundService){
            ...
            if (serviceInfo) registry.unregisterObject({"id" :serviceInfo.id, "api":serviceInfo.api});
            foundService.params = params;
            PzpCommon.wUtil.webinosService.loadServiceModule(foundService, registry, rpcHandler);
            PzpCommon.fs.writeFileSync(PzpCommon.path.resolve(foundService.path, "config.json"), JSON.stringify(params, null, "  "));
            return true;
        }
