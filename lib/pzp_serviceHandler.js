    ...
    function setServiceCache(newModules) {
        ...
        if (newModules.length !== serviceCache.length) {
            for (i = 0 ; i < newModules.length; i ++) {
                newService = newModules[i];
                var alreadyPresent = -1,  itemsFromThisPzp =0;
                for (j =0 ; j < serviceCache.length; j++){
                    if (serviceCache[j].serviceAddress === PzpObject.getSessionId()){
                        if (serviceCache[j].name === newService.apiURI){
                            alreadyPresent = 1;
                        }
                        itemsFromThisPzp++;
                    }
                }
                if (newService.serviceAddress === PzpObject.getSessionId()) {
                    if (alreadyPresent === -1)  {
            ...
            if (newModules.length < itemsFromThisPzp) {
                var len = serviceCache.length;
                for (j =0 ; j < len; j++){
                    if (serviceCache[j].serviceAddress === PzpObject.getSessionId()){
                        alreadyPresent = -1;
                        for (i = 0 ; i < newModules.length; i ++) {
                            if (serviceCache[j].api === newModules[i].apiURI){
                                alreadyPresent = 1;
                            }
                        }
                        if (alreadyPresent === -1 && serviceCache[j].api !=="Dashboard"
                            && serviceCache[j].api !=="http://webinos.org/api/zonenotifications"
                            && serviceCache[j].api !=="ServiceDiscovery"
                            && serviceCache[j].api !=="ServiceConfiguration"){
                            serviceCache.splice(j, 1);
                            len--;
                        }
                    }
                }
