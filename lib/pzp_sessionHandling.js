...
function Pzp(inputConfig) {
    ...
    this.checkTrustedList = function(from) {
        var list = from.split("/"); // Replace with Christos stuff later
        if(list.length === 3) from = list[0]+ "/"+ list[1];
        return (config && config.trustedList && (config.trustedList.pzh.hasOwnProperty(from) || config.trustedList.pzp.hasOwnProperty(from)));
    };
