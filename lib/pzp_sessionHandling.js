...
function Pzp(inputConfig) {
    ...
    this.registerDevice = function (from, to, payload) {
        ...
        cert.generateSignedCertificate(config.cert.internal.conn.csr, function(status, signedCert) {
            if(status) {
    ...
    function initializePzp() {
        try {
            PzpCommon.wUtil.webinosHostname.getHostName(inputConfig.sessionIdentity, function (hostname) {
                ...
                inputConfig.sessionIdentity = hostname;
                config = new PzpCommon.wUtil.webinosConfiguration("Pzp", inputConfig);// sets configuration
                cert = new PzpCommon.wCertificate(config.metaData, config.userData);
                config.cert = cert;
                if(config.createOrLoadWebinosConfiguration()){
                    ...
                    if (config.cert.internal.master === "{}") {
                        var cn = config.metaData.webinosType + "CA:" + config.metaData.webinosName;
                        masterCert = cert.generateSelfSignedCertificate (config.metaData.webinosType+"CA", cn);
                        if(masterCert) {
                            ...
                            csr = cert.generateSelfSignedCertificate(config.metaData.webinosType,cn);
