...
function Pzp(inputConfig) {
    ...
    this.updateExternalCertificates = function(value) {config.cert.external = value; config.storeDetails(PzpCommon.path.join("certificates","external"), "certificates",config.cert.external);};
    ...
    function getCAList(){
        ...
        PzpObject.getExternalCertificates().forEach(function(name){
            caList.push(PzpObject.getExternalCertificates(name).cert);
        });
        return caList;
    }
