...
function Pzp(inputConfig) {
    ...
    this.getServiceCache        = function()   {
        // Reduce current cache to only remote services, then re-add all current
        // local services.
        var remoteCache = config.serviceCache.filter(function(svc) {
          return svc.serviceAddress !== PzpObject.getSessionId();
        });

        // Add our own services.
        var serviceCache = remoteCache.concat(PzpObject.getServices());

        serviceCache.sort(function(a,b) {
...
//                                                            ((b.id) > (a.id)) ? -1 : 0);} );
    ...
    /**
    * Issue 'fetch' requests for remote service proxy javascript if the service
    * is not installed locally.
    */
    this.fetchProxyJS = function() {
      if (PzpObject.getState("Pzh") === "connected") {
        // Find any remote services for which there is no javascript proxy.
        var remoteServiceCache = PzpObject.getServiceCache().filter(function(svc) { return svc.serviceAddress !== PzpObject.getSessionId(); });
        var ownServices = PzpObject.getServices();
        var notPresentServices = [];
        remoteServiceCache.forEach(function(service){
          // Check if the remote service is also installed locally.
          var found = false;
          ownServices.some(function(ownService){
          ...
          if (found !== true){
            // Service not found locally - check if proxy javascript file has already been downloaded.
            var fileNames = PzpCommon.fs.readdirSync(PzpCommon.path.join(__dirname, "../", "wrt"));
            fileNames.sort();
    ...
    this.updateStoreServiceCache = function(serviceCache) {
        ...
        // Fetch any missing remote proxies.
        PzpObject.fetchProxyJS();

        // Reset updated remote service cache, don't merge with existing cache.
        var remoteServiceCache = serviceCache.filter(function(svc) { return svc.serviceAddress !== PzpObject.getSessionId(); });
        PzpObject.addRemoteServices(remoteServiceCache, false);
    };
    /**
