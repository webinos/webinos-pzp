...
var PzpConnectHub = function () {
    ...
    function retryPZHConnection (reset) {
      ...
      if (pzhReconnectTimer === null) {
        ...
        if (PzpObject.getEnrolledStatus()) {
          ...
          logger.log("waiting " + pzhReconnectTimeout/1000 + "secs to connect " +
          PzpObject.getMetaData("serverName")+":"+PzpObject.getWebinosPorts("provider")
    ...
    function pingPzh() {
        var pingHelper = function() {
            ...
            pzhPingSocket = PzpCommon.net.createConnection(PzpObject.getWebinosPorts("provider"),
            ...
            pzhPingSocket.on('error', function() { // Assuming this will happen as internet is not reachable
                ...
                if (pzhReconnectTimer != null) retryPZHConnection(true);
            });
        };
