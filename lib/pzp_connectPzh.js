...
var PzpConnectHub = function () {
    ...
    function connectionWithPzhChecker() {
        var id = setInterval(function(){
            var socket = PzpCommon.net.createConnection(PzpObject.getWebinosPorts().provider,  PzpObject.getServerAddress());
    ...
    this.connectHub = function () {
        try {
            ...
            var socket = PzpCommon.net.createConnection(PzpObject.getWebinosPorts().provider,  PzpObject.getServerAddress()); //Check if we are online..
            socket.setTimeout(10);
            socket.on('connect', function() {
                socket.end();
                ...
                pzpClient = PzpCommon.tls.connect(PzpObject.getWebinosPorts().provider,
                    ...
                    PzpObject.setConnectionParameters(), function() {
                        if (pzpClient.authorized) {
                            PzpObject.handleAuthentication("hub", PzpObject.getPzhId(), pzpClient);
                ...
                pzpClient.on ("end", function() {
                    if (pzpClient.id) {
                        ...
                        PzpObject.cleanUp(pzpClient.id);
                    }
                    retryConnecting();
                });
