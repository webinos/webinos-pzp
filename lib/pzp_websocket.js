...
function PzpWebSocketServer(){
    ...
    if (process.platform == "android") {
        try {
            ...
            wrtServer = require("bridge").load(require("../config.json").android.WebinosSocketServerImpl, exports);
        } catch (e) {
            logger.error ("exception attempting to open wrt server " + e);
        }
    }

    var moduleHttpHandlers = {};
    this.moduleHttpHandlers = moduleHttpHandlers;
                ...
                if (wsServer) {
                    ...
                    if (process.platform == 'android') {
                        ...
                        if(widgetLibrary) {
                            ...
                            process.env.WRT_HOME = require("../config.json").android.wrt;
                            var bridgewm = require('bridge').load(require("../config.json").android.WidgetManagerImpl, exports);
                    ...
                    if (process.platform == 'android') {
                        try {
                            ...
                            // Call interface to webinos-android
                            var notification = bridge.load(require("../config.json").android.PZPNotificationManagerImpl, exports);
