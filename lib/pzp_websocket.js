...
function PzpWebSocketServer(){
    ...
    function getConnectedInfo() {
        var list = [], store = {}, key, key1, zonePzh, zonePzp, externalPzh, externalPzp;
        ...
        if(Object.keys(zonePzh).length === 0) {
             for (key1 in zonePzp) {
                if (zonePzp.hasOwnProperty(key1)){
                    list.push(zonePzp[key1].friendlyName || key1);
                }
            }
            ...
            if(zonePzh.hasOwnProperty(key)) {
                ...
                for (key1 in zonePzp) {
                    if (zonePzp.hasOwnProperty(key1) && key1.split("/") && key1.split("/")[0] === key){
                        store.pzp.push(zonePzp[key1].friendlyName || key1);

                    }
                }
                for (key1 in externalPzp) {
                    if (externalPzp.hasOwnProperty(key1) && key1.split("/") && key1.split("/")[0] === key){
                        store.pzp.push(externalPzp[key1] || key1);

                    }
                }
                if (key === PzpObject.getPzhId()){
                    store.pzp.push(PzpObject.getFriendlyName() || PzpObject.getSessionId());
                }
                list.push(store);
            }
          }
    ...
    function gatherTestPageLinks(msg) {
        ...
        PzpObject.sendConnectedApp (msg.from, prepMsg(msg.from, "gatherTestPageLinks", testLink));
    }
    ...
    this.connectedApp = function(connection, webAppName) {
        ...
        if (connection) {
            var sessionWebApp = null;
            if (!webAppName) {
                webAppName = require("crypto").randomBytes(3).toString("hex").toUpperCase();
            } else if (webAppName.indexOf('localhost') != -1) {
                // widget path syntax:
                // http://localhost:53510/widget/<install_id>/<session_id>/file.
                var items = webAppName.split('/');
                if (items.length == 7) {
                    sessionWebApp = 'IID'+items[4];
                }
            }
            if (sessionWebApp == null) {
                sessionWebApp = require("crypto").createHash("md5").update(PzpObject.getSessionId() + webAppName).digest("hex");
            }
            appId = PzpObject.getSessionId()  + "/"+ sessionWebApp +"_" + Math.round(Math.random()*100);
            ...
            payload = { "pzhId":(PzpObject.getPzhId() && PzpObject.getConnectedPzh().hasOwnProperty(PzpObject.getPzhId()) &&
                                 PzpObject.getConnectedPzh()[PzpObject.getPzhId()].friendlyName)|| PzpObject.getPzhId() || "",
