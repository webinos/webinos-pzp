...
function PzpWebSocketServer(){
    ...
    function getPzpDetails(zone, key) {
        ...
        zone.forEach(function(name){
            if (name.hasOwnProperty(key1) && key === key1.split("/")[0]){
                list.push(name);
            }
        });
        return list;
        ...
        zonePzh.forEach(function(name){
            for (key in name) {
                if(name.hasOwnProperty(key)) {
                    store = {id: key, friendlyName: name[key].friendlyName, pzp:getPzpDetails(zonePzp, key) };
                    if (key === PzpObject.getPzhId()) store.pzp.push({friendlyName: PzpObject.getFriendlyName(), id: PzpObject.getSessionId()});
                    list.push(store);
                }
            }
        });
