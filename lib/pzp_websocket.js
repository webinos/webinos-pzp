...
function PzpWebSocketServer(){
    ...
    function handleHttpRequest(url, req, res) {
        ...
        if (parts.length >= 2 && parts[0] == "module" && PzpObject.moduleHttpHandlers[parts[1]]) {
            ...
            var documentRoot = PzpCommon.path.join (__dirname, "../","node_modules");
            // If we detect that user has not configured PZP, redirect towards the page.
            PzpCommon.wUtil.webinosContent.sendFile(res, documentRoot, path, "index.html");
            return;
        }
                ...
                if (wsServer) {
                    ...
                    if (process.platform == 'android') {
                        ...
                        if(widgetLibrary) {
                            ...
                            process.env.WRT_HOME = androidInterfaces.wrt;
