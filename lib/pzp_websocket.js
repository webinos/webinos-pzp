...
function PzpWebSocketServer(){
    ...
    this.connectedApp = function(connection, webAppName) {
        ...
        if (connection) {
            ...
            PzpObject.prepMsg(appId, "registeredBrowser", payload);
            msg = PzpObject.messageHandler.createRegisterMessage(appId, PzpObject.getSessionId());
            ...
            for (key in connectedWebApp) {
                if (connectedWebApp.hasOwnProperty (key)) {
                    ...
                    msg = PzpObject.messageHandler.createRegisterMessage(key, PzpObject.getSessionId());
                    PzpObject.messageHandler.onMessageReceived(msg, msg.to);
                }
            }
        }
            ...
            if (status) {
                ...
                wsServer.on ("request", function (request) {
                    ...
                    if (request.host && request.host.split (":") &&
                        ...
                        connection.on ("close", function (reason, description) { PzpObject.wsClose (connection, description) });
                    } else {
