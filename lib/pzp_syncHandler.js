...
//                var pzhCache = syncInstance.getObjectHash(receivedMsg);
                ...
                var text = "";
                if (receivedMsg !== {}) {
                    console.log(list["serviceCache"])
                    syncInstance.applyObjectContents(list, receivedMsg, PzpObject.deletedItem); // After this step list is combined copy of local and remote copy
                    console.log(list["serviceCache"])
                    for (var key in list){
