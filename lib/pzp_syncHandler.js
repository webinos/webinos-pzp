    ...
    PzpObject.synchronization_update = function(receivedMsg, id) {
        try {
            if(syncInstance) {
                ...
                var beforeApply = syncInstance.getObjectHash(receivedMsg);
                var inNeedOfSync = 0;
                var newList={};
                var text = "";
                if (receivedMsg !== {}) {
