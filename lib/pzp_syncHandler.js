        ...
        try {
            if(syncInstance) {
                ...
                if (receivedMsg !== {}) {
                    ...
                    for (var key in list){
                        if (list.hasOwnProperty(key)){
                            if (key === "trustedList"&& JSON.stringify(PzpObject.getTrustedList()) !== JSON.stringify(list[key])) {
                                ...
                                newList[key] = PzpObject.getTrustedList();
                                inNeedOfSync = 1;
                                ...
                                newList[key] = PzpObject.getServiceCache();
                                inNeedOfSync = 1;
                    ...
                    logger.log ("Synchronised with the PZH");
                }
              ...
              /*  var inNeedOfSync = 0;
                ...
                for ( key in checkChanges ){
                    if (checkChanges.hasOwnProperty(key) && key !== "connectedDevices" && checkChanges[key]!==storeLastDiffHash[key]) {
                        console.log(key, storeLastDiffHash, storeLastDiffHash[key], checkChanges[key])
