...
var PzpSync = function () {
    ...
    PzpObject.synchronization_compareHash = function(receivedMsg) {
        try{
            if (syncInstance) {
                var list_ = syncInstance.compareObjectHash(prepareSyncList(), receivedMsg.payload.message);
                if (list_.length !== 0) PzpObject.prepMsg(receivedMsg.from, "syncCompare", list_);
            }
        } catch(err) {
    ...
    PzpObject.synchronization_update = function(receivedMsg) {
        try {
            if(syncInstance) {
                ...
                if (receivedMsg !== {}) {
                    ...
                    for (var key in list){
                        if (list.hasOwnProperty(key)){
                            ...
                            } else if (key === "certificates" && JSON.stringify(PzpObject.getExternalCertificates()) !== JSON.stringify(list[key])) {
                ...
                //console.log("PZP CACHE",syncInstance.getObjectHash(prepareSyncList()), PzpObject.getServiceCache());
            }
        } catch(err){
            logger.error(new Error().stack);
        }
