...
var PzpReceiveMessage = function () {
    ...
    this.wsMessage = function (connection, origin, utf8Data) {
        ...
        if(msg.to) msg.to = PzpObject.convertFriendlyNameToSessionId(msg.to);
        if(msg.from) msg.from = PzpObject.convertFriendlyNameToSessionId(msg.from);
        if(msg.resp_to) msg.resp_to = PzpObject.convertFriendlyNameToSessionId(msg.resp_to);
