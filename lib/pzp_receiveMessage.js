    ...
    function fetchJS(msg){
        ...
        for (var key in serviceCache){
            if( serviceCache.hasOwnProperty(key)) {
                var serviceLooked;
                if (serviceCache[key] && msg.payload && msg.payload.message &&
                    serviceCache[key].apiURI.length < msg.payload.message.length) {
                    serviceLooked = msg.payload.message.substr(0, serviceCache[key].apiURI.length);
                } else {
                    serviceLooked =  msg.payload.message;
                }
                if (serviceCache[key].apiURI === serviceLooked){// for dynamic services
                    var fileName =
                        PzpCommon.fs.readdirSync(PzpCommon.path.join(serviceCache[key].path, "wrt"));
                    fileName.sort();
                    for (var j=0; j < fileName.length; j = j + 1) {
                        var stat =
                            PzpCommon.fs.statSync(PzpCommon.path.join(serviceCache[key].path, "wrt",
                                                  fileName[j]));
                        if (!stat.isFile()) continue;
                        try {
                            var data = PzpCommon.fs.readFileSync(
                                PzpCommon.path.join(serviceCache[key].path, "wrt", fileName[j]));
                            var serviceName = serviceCache[key].apiURI.split("/");
                            serviceName = serviceName[serviceName.length-1];
                            PzpObject.prepMsg (msg.from, "clientJS", {name: serviceName, js: data.toString()});
                        } catch(err) {
                            logger.log("Webinos module without client side code. "
                                ,"Using Web RunTime you will not be able to access module "+
                                 serviceCache[key].name);
                        }
                    }
