...
var PzpClient = function () {
     ...
     */
    this.connectPeer = function (entityName, address) {
        var options = PzpObject.setConnectionParameters();
        entityName = PzpObject.getPzhId() + "/"+ entityName;
        options.ca = PzpObject.getCertificate("pzh");
        options.ca.push(PzpObject.getSignedCertificateObj()[entityName]);
        options.servername = entityName;
        ...
        client.on("error", function (err) {
            PzpObject.emit("PZP_CONNECTION_FAILED", err.stack);
        });
    }
