...
(function () {
    ...
    function createCommChannel (successCB) {
        ...
        if (typeof WebinosSocket !== 'undefined') { // Check if we are inside Android widget renderer.
            ...
            try {
                channel = new webSocketClient();
                channel.connect("ws://" + hostname + ":" + port);
        ...
        channel.on("connect",function(connection) {
            ...
            webinos.session.message_send({type: 'prop', payload: {status:'registerBrowser', value: __dirname}});
        });
    }
    ...
    webinos.discovery = new exports.ServiceDiscovery (webinos.rpcHandler);
